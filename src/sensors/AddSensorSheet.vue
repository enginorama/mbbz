...
function addSensor(): void {
  emit('update:value', { sensorId: sensorId.value, vPin: vPin.value, pullUp: pullUp.value });
  open.value = false;
}
...
<template>
  <AppSheet
    title="Add Sensor"
    description="Add a new sensor to the command station."
    v-model:open="open"
  >
      ...
      <Field orientation="horizontal">
        ...
        <Button
          ...
          @click="
            ...
            open = false;
          "
          >{{ $t('globals.formActions.cancel') }}</Button
        >
      </Field>
    </form>
  </AppSheet>
